 ...
 * 
 * Standard doubly-linked circular list insertion operation.
 * This function is used for managing free page lists in the buddy allocator.
 * 
 * UEFI Specification 2.11, Section 7.2 (Memory Allocation Services), Page 151:
 * "The way in which these functions are used is directly related to an important
 *  feature of UEFI memory design. This feature, which stipulates that EFI firmware
 *  owns the system's memory map during preboot, has three major consequences:
 *  • During preboot, all components (including executing EFI images) must cooperate
 *    with the firmware by allocating and freeing memory from the system with the
 *    functions EFI_BOOT_SERVICES.AllocatePages(), EFI_BOOT_SERVICES.AllocatePool(),
 *    EFI_BOOT_SERVICES.FreePages(), and EFI_BOOT_SERVICES.FreePool(). The firmware
 *    dynamically maintains the memory map as these functions are called."
 */
inline static void __attribute__((always_inline))
 ...
 * 
 * Standard doubly-linked circular list deletion operation.
 * Removes element from list and initializes it as empty list.
 * 
 * UEFI Specification 2.11, Section 7.2 (Memory Allocation Services), Page 151:
 * "The firmware dynamically maintains the memory map as these functions are called."
 * This function is used to manage free page lists as part of the memory allocation system.
 */
 ...
 * 
 * Implementation follows buddy allocator algorithm where each child
 * represents half of parent's memory region. Child class is parent->class - 1,
 * maintaining power-of-2 size hierarchy.
 * 
 * UEFI Specification 2.11, Section 7.2.3 (GetMemoryMap), Page 157:
 * "PhysicalStart - Physical address of the first byte in the memory region.
 *  PhysicalStart must be aligned on a 4 KiB boundary, and must not be above
 *  0xfffffffffffff000. Type EFI_PHYSICAL_ADDRESS is defined in the AllocatePages()
 *  function description."
 * 
 * The buddy allocator ensures all allocations are power-of-2 aligned, satisfying
 * this requirement while providing efficient memory management.
 */
static struct Page *
...
/*
 * Attach memory region to physical memory tree.
 * 
 * Splits region into power-of-2 aligned pages and attaches them to the tree.
 * 
 * UEFI Specification 2.11, Section 7.2.3 (GetMemoryMap), Page 157:
 * "PhysicalStart - Physical address of the first byte in the memory region.
 *  PhysicalStart must be aligned on a 4 KiB boundary, and must not be above
 ...
 *  function description."
 * 
 * "NumberOfPages - Number of 4 KiB pages in the memory region. NumberOfPages must
 *  not be 0, and must not be any value that would represent a memory page with a
 *  start address, either physical or virtual, above 0xfffffffffffff000"
 * 
 * Implementation: Aligns region boundaries to 4 KiB (CLASS_SIZE(0)) and splits
 * into maximum possible power-of-2 sized pages, ensuring efficient memory usage
 * while maintaining alignment requirements.
 */
static void
attach_region(uintptr_t start, uintptr_t end, enum PageState type) {
    ...
    /* Align to 4 KiB boundary per UEFI spec Section 7.2.3, Page 157 */
    start = ROUNDDOWN(start, CLASS_SIZE(0));
    end = ROUNDUP(end, CLASS_SIZE(0));

    while (start < end) {
        /* Find maximum class that fits in remaining region and is aligned */
        int class = 0;
        while (class < MAX_CLASS - 1 && 
               start + CLASS_SIZE(class + 1) <= end &&
               !(start & CLASS_MASK(class + 1))) {
            class++;
        }
        
        /* Attach page of found class to tree */
...
/*
 * Display free memory pages grouped by allocation class.
 * 
 * Debug function that iterates through free_classes arrays and prints
 * physical addresses of free pages for each class size.
 * Used by monitor command 'memory' to show current memory allocation state.
 * 
 * This function displays the state of the buddy allocator's free lists,
 * which are organized according to UEFI Specification 2.11, Section 7.2,
 * Page 151, which describes memory allocation services and the management
 * of free memory regions.
 */
void
 ...
 * 
 * UEFI Specification 2.11, Section 7.2.3 (GetMemoryMap), Page 157:
 * "The GetMemoryMap() function returns a copy of the current memory map. The map
 *  is an array of memory descriptors, each of which describes a contiguous block
 *  of memory. The map describes all of memory, no matter how it is being used.
 *  That is, it includes blocks allocated by EFI_BOOT_SERVICES.AllocatePages() and
 *  EFI_BOOT_SERVICES.AllocatePool(), as well as blocks that the firmware is using
 *  for its own purposes."
 * 
 * Section 7.2.3, Page 157:
 * "PhysicalStart - Physical address of the first byte in the memory region.
 *  PhysicalStart must be aligned on a 4 KiB boundary, and must not be above
 *  0xfffffffffffff000. Type EFI_PHYSICAL_ADDRESS is defined in the AllocatePages()
 *  function description."
 ...
 *  start address, either physical or virtual, above 0xfffffffffffff000"
 * 
 * Section 7.2, Page 152, Table 7.9:
 * "EfiConventionalMemory - Free (unallocated) memory."
 * 
 * Section 7.2.3, Page 158:
 * "EFI_MEMORY_WB - Memory cacheability attribute: The memory region supports being
 *  configured as cacheable with a 'write back' policy. Reads and writes that hit
 *  in the cache do not propagate to main memory. Dirty data is written back to
 *  main memory when a new cache line is allocated."
 * 
 * Section 7.4.6 (ExitBootServices), Page 203:
    ...
    if (uefi_lp && uefi_lp->MemoryMap) {
        ...
        while (start < end) {
            ...
            case EFI_CONVENTIONAL_MEMORY:
                /* UEFI Specification 2.11, Section 7.2.3, Page 158:
                 * "EFI_MEMORY_WB - Memory cacheability attribute: The memory region supports
                 *  being configured as cacheable with a 'write back' policy. Reads and writes
                 *  that hit in the cache do not propagate to main memory. Dirty data is written
                 *  back to main memory when a new cache line is allocated."
                 * Use this attribute to determine if memory is allocatable. */
            ...
            /* Attach memory described by memory map entry.
             * 
             * UEFI Specification 2.11, Section 7.2.3 (GetMemoryMap), Page 157:
             * "PhysicalStart - Physical address of the first byte in the memory region.
             *  PhysicalStart must be aligned on a 4 KiB boundary, and must not be above
             ...
             *  start address, either physical or virtual, above 0xfffffffffffff000"
             * 
             * The region size is calculated as start->PhysicalStart + start->NumberOfPages * EFI_PAGE_SIZE,
             * where EFI_PAGE_SIZE = 4096 bytes (4 KiB) as defined in inc/uefi.h */
